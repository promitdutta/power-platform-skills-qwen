...
The generated service file (`src/generated/services/Office365OutlookService.ts`) is large. **Use `grep_search` to find specific methods** instead of reading the entire file:

```
grep_search pattern="async \w+" path="src/generated/services/Office365OutlookService.ts"
```
...
| Type                                                       | Purpose                                                                                                |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `CalendarEventClientReceiveStringEnums`                    | read_file model -- has `Subject`, `Start`, `End`, `Id`, `ShowAs`, `IsAllDay`, `Organizer`                   |
| `CalendarEventHtmlClient`                                  | write_file model -- requires `Subject`, `Start`, `End`; optional `Body`, `ShowAs`, `Importance`, `Reminder` |
