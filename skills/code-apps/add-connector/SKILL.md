...
After adding, inspect the generated files. **Generated service files can be very large** -- use `grep_search` to find specific methods instead of reading the entire file:

```
grep_search pattern="async \w+" path="src/generated/services/<Connector>Service.ts"
```
...
1. grep_search for the method name to find its signature
2. read_file just that method's section (use `offset` and `limit` parameters on read_file)
