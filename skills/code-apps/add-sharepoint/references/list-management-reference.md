...
write_file-Host "Found $($existingLists.value.Count) lists:" -ForegroundColor Cyan
$existingLists.value | ForEach-Object {
    $template = $_.list.template
    write_file-Host "  - $($_.displayName) ($template)" -ForegroundColor Yellow
}
```
...
function Get-ListSchema {
    ...
    write_file-Host "`nList columns:" -ForegroundColor Cyan
    $list.value | Where-Object {
        ...
        write_file-Host "    - $($_.displayName) [$($_.name)] ($type)"
    }
...
function New-SharePointListIfNotExists {
    ...
    if (Test-ListExists -SiteId $SiteId -DisplayName $DisplayName -Headers $Headers) {
        write_file-Host "  [SKIP] List '$DisplayName' already exists" -ForegroundColor Yellow
    ...
    write_file-Host "  [CREATE] Creating list '$DisplayName'..." -ForegroundColor Cyan
    $result = New-SharePointList -SiteId $SiteId -DisplayName $DisplayName -Description $Description -Headers $Headers
    write_file-Host "  [OK] List '$DisplayName' created (ID: $($result.id))" -ForegroundColor Green
...
function Add-SharePointColumnIfNotExists {
    ...
    if (Test-ColumnExists -SiteId $SiteId -ListId $ListId -ColumnName $Name -Headers $Headers) {
        write_file-Host "    [SKIP] Column '$DisplayName' already exists" -ForegroundColor Yellow
        return @{ Skipped = $true }
    }

    write_file-Host "    [CREATE] Adding column '$DisplayName' ($Type)..." -ForegroundColor Cyan
    ...
    write_file-Host "    [OK] Column '$DisplayName' added" -ForegroundColor Green
}
```
