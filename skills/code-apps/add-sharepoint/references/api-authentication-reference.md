...
function Initialize-SharePointGraphApi {
    ...
    write_file-Host "Connected to site: $SiteUrl" -ForegroundColor Green
    write_file-Host "Site ID: $siteId" -ForegroundColor Cyan
...
function Invoke-GraphApi {
    ...
    try {
        ...
        if ($_.Exception.Response.StatusCode -eq 401) {
            write_file-Host "Token expired, refreshing..." -ForegroundColor Yellow
...
try {
    ...
    write_file-Host "Connected to: $($site.displayName)" -ForegroundColor Green
    write_file-Host "Web URL: $($site.webUrl)" -ForegroundColor Cyan
} catch {
    write_file-Host "Connection failed: $($_.Exception.Message)" -ForegroundColor Red
}
```
...
- **Sites.read_file.All** - read_file site and list metadata (minimum for discovery)
