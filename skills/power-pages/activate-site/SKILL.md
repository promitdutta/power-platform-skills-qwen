...
user-invocable: true
allowed-tools: read_file, run_shell_command, glob, grep_search, ask_user_question, todo_write, todo_write, todo_write
model: sonnet
---
...
read_file the file and extract the `siteName` field. If not found, ask the user for the site name using `ask_user_question`.
...
```powershell
node "${extensionPath}/skills/activate-site/scripts/generate-subdomain.js"
```
...
Present the generated subdomain to the user and ask them to accept or enter their own using `ask_user_question`:
...
Present all activation parameters to the user using `ask_user_question`:
...
```powershell
node "${extensionPath}/skills/activate-site/scripts/activate-site.js" --siteName "<siteName>" --subdomain "<subdomain>" --organizationId "<organizationId>" --environmentId "<environmentId>" --cloud "<cloud>" --websiteRecordId "<websiteRecordId>"
```
...
> **Note:** This script may run for up to 5 minutes while polling. Use a run_shell_command timeout of at least 360 seconds (6 minutes).
...
Use `todo_write` at the start to track progress through each phase:
...
Mark each task complete with `todo_write` as you finish each phase.
