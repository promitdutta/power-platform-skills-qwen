...
user-invocable: true
allowed-tools: read_file, write_file, edit, grep_search, glob, run_shell_command, ask_user_question, Task, todo_write, todo_write, todo_write, mcp__plugin_power-pages_playwright__browser_navigate, mcp__plugin_power-pages_playwright__browser_snapshot, mcp__plugin_power-pages_playwright__browser_click, mcp__plugin_power-pages_playwright__browser_close
model: sonnet
---
...
read_file `powerpages.config.json` to get the site name and config:
...
read_file `package.json` to determine the framework and locate key files. See `${extensionPath}/references/framework-conventions.md` for the full framework → public directory → index HTML mapping and route discovery patterns.
...
Use `ask_user_question` to collect SEO preferences:
...
After presenting the plan, use `ask_user_question` to get approval:
...
Update each task with `todo_write` as it is completed.
