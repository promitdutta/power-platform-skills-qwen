...
> **Authentication, error handling, and retry patterns** are in the shared reference: `${extensionPath}/references/odata-common.md`. read_file that file first for headers, token refresh, HTTP status codes, and retry logic.
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "EntityDefinitions(LogicalName='cr123_project')?\$select=EntitySetName"
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "cr123_projects" --body '{"cr123_name":"Website Redesign","cr123_description":"Modernize the corporate website with a fresh design","cr123_startdate":"2025-06-15T10:30:00Z","cr123_budget":15000.00,"cr123_isactive":true,"cr123_status":100000000}'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "cr123_projects" --body '{"cr123_name":"Website Redesign"}' --include-headers
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "EntityDefinitions(LogicalName='cr123_project')/Attributes(LogicalName='cr123_status')/Microsoft.Dynamics.CRM.PicklistAttributeMetadata?\$expand=OptionSet"
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "cr123_tasks" --body '{"cr123_name":"Design mockups","cr123_duedate":"2025-07-01T00:00:00Z","cr123_ProjectId@odata.bind":"/cr123_projects(<projectGuid>)"}' --include-headers
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "<EntitySetName>" --body '{"cr123_name":"Project Update Meeting","cr123_ProjectId@odata.bind":"/cr123_projects(<projectGuid>)","cr123_ContactId@odata.bind":"/contacts(<contactGuid>)"}' --include-headers
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "cr123_projects?\$count=true&\$top=0"
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "\$batch" --body '<batch body>'
```
...
See `${extensionPath}/references/odata-common.md` for HTTP status codes, error response format, Dataverse error codes, and retry patterns.
