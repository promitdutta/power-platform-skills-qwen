...
allowed-tools: read_file, write_file, run_shell_command, glob, grep_search, ask_user_question, todo_write, todo_write, todo_write, Agent
model: opus
---
...
2. **Gather Configuration** — read_file all web roles, table permissions, and site code
...
At the start of Step 1, create all tasks upfront using `todo_write`. Mark each task `in_progress` when starting and `completed` when done.

| Task subject | activeForm | Description |
|-------------|------------|-------------|
| Verify site deployment | Verifying site deployment | Check .powerpages-site folder and table permissions exist |
| Gather configuration | Gathering configuration | read_file web roles, table permissions, and site code |
...
### 2.1 read_file Web Roles

read_file all files matching `**/.powerpages-site/web-roles/*.yml`. Extract `id`, `name`, `anonymoususersrole`, `authenticatedusersrole` from each.

### 2.2 read_file Table Permissions

read_file all files matching `**/.powerpages-site/table-permissions/*.tablepermission.yml`. For each permission, extract:
...
```powershell
$schemaValidation = node "${extensionPath}/scripts/validate-permissions-schema.js" --projectRoot "<PROJECT_ROOT>"
```
...
```powershell
$schemaValidation = node "${extensionPath}/scripts/validate-permissions-schema.js" --projectRoot "<PROJECT_ROOT>" --validate-dataverse-relationships --envUrl "$envUrl"
```
...
```powershell
$lookups = node "${extensionPath}/skills/audit-permissions/scripts/query-table-lookups.js" --envUrl "$envUrl" --table "<table_logical_name>"
```
...
```powershell
$rels = node "${extensionPath}/skills/audit-permissions/scripts/query-table-relationships.js" --envUrl "$envUrl" --table "<parent_table>"
```
...
Use `todo_write` at any point to review progress and see which tables still need auditing.
...
- If `write: true` but `read: false` → finding:
  - **Severity:** `warning`
  - **Title:** `write_file enabled without read on <table>`
...
- If `write: true` but no write patterns in code → finding:
  - **Severity:** `info`
  - **Title:** `write_file enabled but not used for <table>`
...
After all checks, mark the table's task as `completed` via `todo_write`.
...
Use `todo_write` to review all completed audits, then mark the "Compile audit findings" task as `in_progress` and proceed to Step 5.
...
write_file a temporary JSON data file (e.g., `<OUTPUT_DIR>/audit-data.json`) with these keys:
...
```json
{
  "name": "Product - Anonymous read_file",
  "table": "cra5b_product",
  "scope": "Global",
...
```powershell
node "${extensionPath}/scripts/render-audit-report.js" --output "<OUTPUT_PATH>" --data "<DATA_JSON_PATH>"
```
...
4. **Ask the user** using `ask_user_question`: "Would you like me to fix any of these issues? I can create or update table permissions to resolve the critical and warning findings."
...
- **read_file-only analysis**: This skill only reads existing configuration and code. It does NOT modify any files unless the user explicitly asks to fix issues.
