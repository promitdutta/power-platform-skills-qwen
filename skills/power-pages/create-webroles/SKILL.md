...
user-invocable: true
allowed-tools: read_file, write_file, run_shell_command, grep_search, glob, ask_user_question, Task, todo_write, todo_write, todo_write
model: opus
---
...
- **Always use the UUID script**: Never generate UUIDs manually — always use `${extensionPath}/scripts/generate-uuid.js` to produce valid UUID v4 values for each web role.
...
2. **Phase 2: Discover Existing Roles** → read_file current web role YAML files
...
2. **If `.powerpages-site` does NOT exist:** Ask the user to deploy first via `ask_user_question` (options: "Yes, deploy now (Recommended)", "No, I'll do it later"). If yes, invoke `/deploy-site` then resume from Phase 2. If no, stop.
...
1. read_file all YAML files in the `.powerpages-site/web-roles/` directory. Each file represents one web role with this format:
...
1. Based on the site's purpose and the existing roles, suggest appropriate web roles. Use `ask_user_question` to confirm with the user.
...
```powershell
node "${extensionPath}/scripts/generate-uuid.js"
```
...
write_file the file with this exact format (4 fields, no extra whitespace or comments):
...
| Discover existing roles | Discovering existing roles | read_file current web role YAML files and compile list of existing roles |
