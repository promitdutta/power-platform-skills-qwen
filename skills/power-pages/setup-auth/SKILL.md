...
user-invocable: true
allowed-tools: read_file, write_file, edit, run_shell_command, grep_search, glob, ask_user_question, Task, todo_write, todo_write, todo_write, skill
model: opus
---
...
read_file each file and compile a list of existing web roles (name, id, flags).
...
Use `ask_user_question` to determine the scope:
...
Reference: `${extensionPath}/skills/setup-auth/references/authentication-reference.md`
...
Reference: `${extensionPath}/skills/setup-auth/references/authorization-reference.md`
...
read_file each file and verify it contains the expected exports and functions:
...
Use `todo_write` at the start to track each phase:
...
Update each task with `todo_write` as phases are completed.
