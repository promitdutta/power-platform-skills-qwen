...
user-invocable: true
allowed-tools: read_file, write_file, edit, run_shell_command, grep_search, glob, ask_user_question, Task, todo_write, todo_write, todo_write
model: opus
---
...
- **Use todo_write/todo_write**: Track all progress throughout all phases — create the todo list upfront with all phases before starting any work.
...
### 1.2 read_file Existing Config

read_file `powerpages.config.json` to get the site name:
...
read_file `package.json` to determine the framework (React, Vue, Angular, or Astro). See `${extensionPath}/references/framework-conventions.md` for the full framework detection mapping.
...
| Categories | `cr4fc_category` | `cr4fc_categories` | read_file | `CategoryFilter.tsx` | None |
...
For each table, use the `Task` tool to invoke the `webapi-integration` agent at `${extensionPath}/agents/webapi-integration.md`:
...
Ask the user how they want to define the permissions using the `ask_user_question` tool:
...
1. Ask the user to provide their permissions diagram. Supported formats:
   - **Image file** (PNG, JPG) — Use the `read_file` tool to view the image and extract web roles, table permissions, CRUD flags, scopes, and site settings from it
...
6. Present the parsed permissions plan to the user for approval using `ask_user_question`:
...
Use the `Task` tool to invoke the `table-permissions-architect` agent at `${extensionPath}/agents/table-permissions-architect.md`:
...
Use the `Task` tool to invoke the `webapi-settings-architect` agent at `${extensionPath}/agents/webapi-settings-architect.md`:
...
```powershell
node "${extensionPath}/skills/create-webroles/scripts/create-web-role.js" --projectRoot "<PROJECT_ROOT>" --name "<Role Name>" [--anonymous] [--authenticated]
```
...
```powershell
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/<table>/enabled" --value "true" --description "Enable Web API access for <table> table" --type "boolean"
```

**Fields setting (string — use the validated column names from the diagram):**

```powershell
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/<table>/fields" --value "<comma-separated-validated-columns>" --description "Allowed fields for <table> Web API access"
```

**Inner error setting (boolean, optional for debugging):**

```powershell
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/error/innererror" --value "true" --description "Enable detailed error messages for debugging" --type "boolean"
```
...
> Reference: `${extensionPath}/references/skill-tracking-reference.md`
...
- **Use todo_write/todo_write** to track progress at every phase
...
Before starting Phase 1, create a task list with all phases using `todo_write`:
...
Mark each task `in_progress` when starting it and `completed` when done via `todo_write`. This gives the user visibility into progress and keeps the workflow deterministic.
