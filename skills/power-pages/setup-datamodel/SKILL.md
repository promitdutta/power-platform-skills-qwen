...
2. Follow the prerequisite steps in `${extensionPath}/references/dataverse-prerequisites.md` to verify PAC CLI auth, acquire an Azure CLI token, and confirm API access. Store the environment URL as `$envUrl`.
...
1. Ask the user how they want to define the data model using the `ask_user_question` tool:
...
1. Ask the user to provide their ER diagram. Supported formats:
   - **Image file** (PNG, JPG) — Use the `read_file` tool to view the image and extract tables, columns, relationships, and cardinalities from it
...
2. Spawn the agent:
   ...
   Task tool:
     ...
     prompt: |
       ...
       ${extensionPath}/agents/data-model-architect.md
...
Use `ask_user_question` to get approval:
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "api/data/v9.2/EntityDefinitions(LogicalName='<table_logical_name>')"
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "api/data/v9.2/EntityDefinitions" --body '<JSON body from references/odata-api-patterns.md>'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "api/data/v9.2/EntityDefinitions(LogicalName='<table>')/Attributes" --body '<column JSON from references/odata-api-patterns.md>'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "api/data/v9.2/RelationshipDefinitions" --body '<relationship JSON from references/odata-api-patterns.md>'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "api/data/v9.2/RelationshipDefinitions" --body '<M:N relationship JSON from references/odata-api-patterns.md>'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> POST "api/data/v9.2/PublishXml" --body '{"ParameterXml":"<importexportxml><entities><entity>cr123_project</entity><entity>cr123_task</entity></entities></importexportxml>"}'
```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "api/data/v9.2/EntityDefinitions(LogicalName='<table>')?$select=LogicalName,DisplayName"
```

### 8.3 write_file Manifest
...
Use the `write_file` tool to create this file at `<PROJECT_ROOT>/.datamodel-manifest.json`. Only include tables and columns that were confirmed to exist in Step 8.2. See `${extensionPath}/references/datamodel-manifest-schema.md` for the full schema specification.
