...
allowed-tools: read_file, write_file, edit, grep_search, glob, run_shell_command, web_search, ask_user_question, Task, todo_write, todo_write, todo_write, mcp__plugin_power-pages_playwright__browser_navigate, mcp__plugin_power-pages_playwright__browser_snapshot, mcp__plugin_power-pages_playwright__browser_click
model: opus
---
...
- **Use real images**: Source high-quality photos from Unsplash wherever pages need visual content — hero sections, feature cards, about pages, backgrounds, etc. Use `https://images.unsplash.com/photo-{id}?w={width}&h={height}&fit=crop` URLs with specific photo IDs found via `web_search`. Never leave image placeholders or broken `<img>` tags pointing to nonexistent files.
...
3. If site purpose is unclear, use `ask_user_question`:
...
> See `${extensionPath}/references/framework-conventions.md` for the full framework → build tool → router → output path mapping.

**Actions**:

### 2.1 Copy Template

> `${extensionPath}` is already resolved to the plugin's absolute path at runtime. Use it directly in glob/read_file paths — do NOT search for the plugin directory.

read_file and copy all files from the matching asset template to the project directory:

| Framework | Asset Directory |
|-----------|----------------|
| React | `${extensionPath}/skills/create-site/assets/react/` |
| Vue | `${extensionPath}/skills/create-site/assets/vue/` |
| Angular | `${extensionPath}/skills/create-site/assets/angular/` |
| Astro | `${extensionPath}/skills/create-site/assets/astro/` |

Use `glob` to discover all files in the asset directory, `read_file` each file, then `write_file` to the project directory preserving the relative path structure.
...
Run `npm run dev` in the background using `run_shell_command` with `run_in_background: true`. Note the local URL (typically `http://localhost:5173` for Vite or `http://localhost:4200` for Angular or `http://localhost:4321` for Astro).
...
1. Use `ask_user_question` to collect feature and design requirements:
...
2. read_file the design aesthetics reference: `${extensionPath}/skills/create-site/references/design-aesthetics.md`
...
1. read_file the design aesthetics reference: `${extensionPath}/skills/create-site/references/design-aesthetics.md`
...
3. Use `ask_user_question` to get approval:
...
1. Use `web_search` to search Unsplash for relevant photos (e.g., `site:unsplash.com modern office workspace`)
...
Run the audit script via `run_shell_command`, passing the dev server URL and all site routes:

```powershell
node "${extensionPath}/skills/create-site/scripts/axe-audit.js" --url <DEV_SERVER_URL> --routes /,/about,/services,/contact --project-root "<PROJECT_ROOT>"
```
...
4. Ask the user to review using `ask_user_question`:
...
2. Use `ask_user_question` with options: **Deploy now (Recommended)**, **Skip for now**:
...
- User approved via ask_user_question
