...
user-invocable: true
allowed-tools: run_shell_command, ask_user_question, read_file, edit, write_file
---
...
2. read_file the existing configuration file at `CONFIG_PATH`, or create a new empty config if it doesn't exist:
   ```json
   {}
   ```
...
5. write_file the updated configuration back to `CONFIG_PATH` with proper JSON formatting (2-space indentation).
