...
user-invocable: true
allowed-tools: read_file, write_file, edit, run_shell_command, Task, todo_write, todo_write, todo_write, mcp__canvas-authoring__sync_canvas, mcp__canvas-authoring__compile_canvas
---
...
3. Create the folder using `run_shell_command`: `mkdir -p <folder-name>`
...
read_file all synced `.pa.yaml` files. Based on `$ARGUMENTS` and the current app state, determine
...
read_file `${extensionPath}/references/TechnicalGuide.md` before making changes.
...
> Plugin root: ${extensionPath}
> Synced files: [list of .pa.yaml filenames]
>
> Follow the instructions in your agent file. write_file canvas-edit-plan.md to the working
...
> - Working directory: [absolute working directory path]
>
> Follow the instructions in your agent file. [edit / write_file] [ScreenName].pa.yaml and return
...
**On failure:** read_file every error in the output. Errors will reference specific files and
line numbers. For each error:

1. `read_file` the referenced `.pa.yaml` file
