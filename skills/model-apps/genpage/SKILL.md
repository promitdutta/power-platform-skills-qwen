...
user-invocable: true
model: sonnet
allowed-tools: read_file, write_file, edit, run_shell_command, glob, grep_search, web_fetch, ask_user_question, todo_write, todo_write, todo_write
---
...
1. **"Create a new generative page or edit an existing one?"** (use `ask_user_question`)
...
2. **"Describe the page you'd like to build"** (use `ask_user_question`) — present two example descriptions as options and let the user type their own via the "Other" option:
...
3. **"Will the page use Dataverse entities or mock data?"** (use `ask_user_question`)
...
4. **"Any specific requirements?"** (use `ask_user_question`) — styling, features (search, filtering, sorting), accessibility, responsive behavior, interactions
...
> **Windows + run_shell_command**: Always use forward slashes in file paths (e.g., `D:/temp/RuntimeTypes.ts`). Backslashes like `\t` or `\R` are consumed as escape sequences by bash, producing wrong paths.
...
### Step 6: read_file Code Generation Rules and Samples
...
After successful deployment, ask the user (use `ask_user_question`):
