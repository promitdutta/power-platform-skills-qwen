...
## Step 1 — read_file the Plan Document

read_file `canvas-edit-plan.md` at the path provided in your invocation prompt.
...
Call `todo_write` for: "edit [Screen Name] screen"

## Step 3 — Apply Changes

**If your action is "Modify" (existing screen):**

read_file the current `[ScreenName].pa.yaml` from the working directory. Then apply each change
...
write_file `[ScreenName].pa.yaml` to the working directory from scratch. Follow the Per-Screen
...
write_file the simplest working version of each formula. The compiler will catch syntax errors —
