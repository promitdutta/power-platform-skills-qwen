...
- The plugin root directory (`${extensionPath}`), from which you must read `references/TechnicalGuide.md` and `references/DesignGuide.md`

## Step 1 — read_file Reference Documents

read_file both reference documents before doing anything else:

- `${extensionPath}/references/TechnicalGuide.md`
- `${extensionPath}/references/DesignGuide.md`
...
## Step 6 — write_file App.pa.yaml

write_file the app-level YAML file (`App.pa.yaml`) to the working directory. This file is shared
across all screens — do not write screen-level content here. Follow TechnicalGuide.md
conventions for app-level properties.

## Step 7 — write_file canvas-app-plan.md

write_file `canvas-app-plan.md` to the working directory. This document is the **single source of
truth** for all `canvas-screen-builder` agents — each builder will only `read_file` this file and
...
Mark the "write_file plan document" task complete when done.
