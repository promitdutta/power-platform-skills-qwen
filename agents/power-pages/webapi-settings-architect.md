...
2. **Discover Existing Site Settings** — read_file existing Web API site settings
...
read_file all Web API-related site settings in `.powerpages-site/site-settings/`:
...
1. **`$select` statements** — Column select arrays in service files:
   ...
   ```text
   grep_search: "\$select|_SELECT" in src/**/*.ts
   ```

2. **POST/PATCH request bodies** — Columns written in create/update operations:

   ```text
   grep_search: "cr[a-z0-9]+_\w+" in src/shared/services/*.ts or src/services/*.ts
   ```

3. **Type definitions** — TypeScript entity interfaces for column names:

   ```text
   grep_search: "cr[a-z0-9]+_\w+" in src/types/*.ts
   ```

4. **`$filter` and `$orderby` clauses** — Columns used in queries:

   ```text
   grep_search: "\$filter|\$orderby" in src/**/*.ts
   ```

5. **File/image upload code** — Columns used in file operations:

   ```text
   grep_search: "uploadFileColumn|uploadFile|upload\w+Photo|upload\w+Image|upload\w+File" in src/**/*.ts
   ```
...
1. **Lookup column references** — OData uses `_<logicalname>_value` format to read lookup GUIDs:
   ...
   ```text
   grep_search: "_cr[a-z0-9]+_\w+_value" in src/**/*.ts
   ```
...
```
node "${extensionPath}/scripts/dataverse-request.js" <envUrl> GET "EntityDefinitions(LogicalName='<table_logical_name>')/Attributes?\$select=LogicalName,DisplayName,AttributeType,IsPrimaryId,SchemaName&\$filter=IsCustomAttribute eq true or IsPrimaryId eq true"
```
...
|------|---------|----------|
| LogicalName | `cr87b_productcategoryid` | write_file operations (setting the lookup value via POST/PATCH) |
| OData computed attribute | `_cr87b_productcategoryid_value` | read_file operations (`$select`, `$filter`, response body GUID) |
...
```powershell
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/<table_logical_name>/enabled" --value "true" --description "Enable Web API access for <table_logical_name> table" --type "boolean"
```

**2. Fields setting:**

```powershell
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/<table_logical_name>/fields" --value "<comma-separated-validated-column-logicalnames>" --description "Allowed fields for <table_logical_name> Web API access"
```
...
# Fields setting
node "${extensionPath}/scripts/create-site-setting.js" --projectRoot "<PROJECT_ROOT>" --name "Webapi/<table>/fields" --value "<validated-columns>" --description "Allowed fields for <table> Web API access"
```
...
- **No manual YAML writes**: Do NOT use `write_file` or `edit` to create YAML files in `.powerpages-site/`. Always use the `create-site-setting.js` script via `run_shell_command`. The script handles all formatting (unquoted booleans, UUIDs, alphabetical fields) automatically.
